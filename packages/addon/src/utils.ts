...
export const logger = (() => {
  ...
  const enableSummary = process.env.EASYNEWS_SUMMARIZE_LOGS?.toLowerCase() === 'true';
  ...
  // Wrap with summary logger if enabled (default to enabled)
  return enableSummary !== false ? new SummaryLogger(baseLogger) : baseLogger;
})();
